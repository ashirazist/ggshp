...
#write test
#unlist and return right result
#proper error report

#proper errors warnings
...
team_11 <- function(path=getwd(), tol=0.1) {
  ...
  geothin <- maptools::thinnedSpatialPoly(
    as(geobig, "Spatial"), tolerance = tol,
  ...
  df<-map_depth(.x = geost$geometry, (nlevel-1), .f =as.data.frame)%>% #change buttom layer into data frame
